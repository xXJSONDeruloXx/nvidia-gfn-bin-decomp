...
# File: _header_value_parser.pyc (Python 3.10)
...
str(x))(self))
    ...
    def value(self):
        return ''.join((lambda .0: for x in .0:
if x.value:
x.valuecontinueNone)(self))

    value = property(value)
    
    def all_defects(self):
        return sum((lambda .0: for x in .0:
x.all_defects)(self), self.defects)
    ...
    def as_ew_allowed(self):
        ...
        return all((lambda .0: for part in .0:
part.as_ew_allowed)(self))
...
class BareQuotedString(QuotedString):
    ...
    def __str__(self):
        return quote_string(''.join((lambda .0: for x in .0:
str(x))(self)))
...
class AddressList(TokenList):
    ...
    def mailboxes(self):
        return sum((lambda .0: for x in .0:
if x.token_type == 'address':
x.mailboxescontinueNone)(self), [])
    ...
    def all_mailboxes(self):
        return sum((lambda .0: for x in .0:
if x.token_type == 'address':
x.all_mailboxescontinueNone)(self), [])
    ...
    def addr_spec(self):
        for x in self:
            if x.token_type == 'addr-spec':
                if x.local_part:
                    return x.addr_spec
                return None(x.local_part) + x.addr_spec
            return '<>'
...
class Parameter(TokenList):
    ...
    def param_value(self):
        for token in self:
            ...
            if None.token_type == 'quoted-string':
                for token in token:
                    if token.token_type == 'bare-quoted-string':
                        for token in token:
                            if token.token_type == 'value':
...
def get_dot_atom_text(value):
    ...
    if None and value[0] not in ATOM_E