...
def _type_convert(arg = None, module = (None,), *, allow_special_forms):
    ...
    if arg is None:
        return type(None)
    if None(arg, str):
        return ForwardRef(arg, module, allow_special_forms, **('module', 'is_class'))
...
def _type_check(arg, msg = None, is_argument = (True, None), module = {
    ...
    arg = _type_convert(arg, module, allow_special_forms, **('module', 'allow_special_forms'))
    if isinstance(arg, _GenericAlias) and arg.__origin__ in invalid_generic_forms:
        raise TypeError(f'''{arg} is not valid as type argument''')
    if None in (Any, NoReturn, Final, TypeAlias):
        return arg
    if None(arg, _SpecialForm) or arg in (Generic, Protocol):
        raise TypeError(f'''Plain {arg} is not valid as type argument''')
    if None(arg, (type, TypeVar, ForwardRef, types.UnionType, ParamSpec, ParamSpecArgs, ParamSpecKwargs)):
        return arg
    if not None(arg):
...
def _collect_type_vars(types_, typevar_types = (None,)):
    ...
    for t in types_:
        ...
        if isinstance(t, (_GenericAlias, GenericAlias, types.UnionType)):
            None((lambda .0 = None: [ t for t in .0 if t not in tvars ])(t.__parameters__))
    return tuple(tvars)
...
def _eval_type(t, globalns, localns, recursive_guard = (frozenset(),)):
    ...
    if None(t, (_GenericAlias, GenericAlias, types.UnionType)):
        ev_args = None((lambda .0 = None: for a in .0:
_eval_type(a, globalns, localns, recursive_guard))(t.__args__))
...
_SpecialForm = <NODE:27>((lambda : __slots__ = ('_name', '__doc__', '_getitem')
...
_LiteralSpecialForm = <NODE:27>((lambda : 
def __getitem__(self, parameters):
if not isinstance(parameters, tuple):
parameters = (parameters,)# WARNING: Decompyle incomplete
), '_LiteralSpecialForm', _SpecialForm, True, **('_root',))
...
def Union(self, parameters):
    ...
    parameters = None((lambda .0 = None: for p in .0:
_type_check(p, msg))(parameters))
...
Literal = _LiteralSpecialForm(_tp_cache(True, **('typed',))(Literal))
